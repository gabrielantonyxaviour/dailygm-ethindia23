...
function requiresPermissions() external view returns (uint8 permissions){
    ...
    function validateUserOp(
        ...
        if (missingAccountFunds != 0) {
            ...
            ISafe(safeAddress).execTransactionFromModule(payable(SUPPORTED_ENTRYPOINT), missingAccountFunds, "", 0);
        }
    }
    ...
    function executeUserOp(address to, uint256 value, bytes memory data, uint8 operation) external onlySupportedEntryPoint {
        require(ISafe(msg.sender).execTransactionFromModule(payable(to), value, data, operation), "Execution failed");
    }
